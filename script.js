...
const resetFlipped = () => {
  flippedCards.forEach((card) => {
    ...
    card.setAttribute("aria-label", "Hidden word");
  });
  flippedCards = [];
  lockBoard = false;
};
...
const onCardClick = (event) => {
  ...
  card.setAttribute("aria-label", card.dataset.word);
  ...
  if (flippedCards.length === 2) {
    ...
    if (first.dataset.word === second.dataset.word) {
      ...
      first.setAttribute("aria-label", first.dataset.word);
      second.setAttribute("aria-label", second.dataset.word);
      flippedCards = [];
      lockBoard = false;
      handleMatch();
    } else {
...
const renderBoard = () => {
  ...
  currentDeck.forEach((word) => {
    ...
    tile.innerHTML = `<span class="card-back">🎈</span><span class="card-front">${word}</span>`;
    tile.dataset.word = word;
    tile.setAttribute("aria-label", "Hidden word");
...
const initGameControls = () => {
  ...
  elements.shuffleGame.addEventListener("click", () => {
    buildDeck();
    if (currentDeck.length > 0) {
      setFocusMode(true);
    }
